        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            for (var i = 0; i < codes.Count; i++)
            {
                ...
                if (i > 5)
                {
                    if
                    (
                        codes[i - 6].opcode == OpCodes.Ldloca_S &&
                        codes[i - 5].opcode == OpCodes.Ldflda &&
                        codes[i - 4].opcode == OpCodes.Dup &&
                        codes[i - 3].opcode == OpCodes.Ldind_R4 &&
                        codes[i - 2].opcode == OpCodes.Ldc_R4 && Convert.ToSingle(codes[i - 2].operand) == floatValue &&
                        codes[i - 1].opcode == OpCodes.Add &&
                        codes[i].opcode == OpCodes.Stind_R4
                    )
                    {
                        yield return new CodeInstruction(OpCodes.Ldloc_1); // 加载局部变量1 (building_bed) 的值
                        yield return new CodeInstruction(OpCodes.Isinst, typeof(Building_BondageBed));
                        yield return new CodeInstruction(OpCodes.Brfalse, label1); // 如果对象引用为空，则跳转到标签 label1
                        //对象引用不为空，可以进行后续操作
                        yield return new CodeInstruction(OpCodes.Ldloc_1);
                        yield return new CodeInstruction(OpCodes.Call, typeof(ThingCompUtility).GetMethod("TryGetComp").MakeGenericMethod(typeof(CompLayerExtension))); // 调用 TryGetComp<CompLayerExtension>() 方法
                        yield return new CodeInstruction(OpCodes.Ldloc_0); // 加载局部变量0 (__result) 的值
                        yield return new CodeInstruction(OpCodes.Callvirt, typeof(CompLayerExtension).GetMethod("ChangePawnDrawOffset")); // 调用 ChangePawnDrawOffset(__result) 方法
                        yield return new CodeInstruction(OpCodes.Stloc_0); // 将结果存储回局部变量0 (__result)
